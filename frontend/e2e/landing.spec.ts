...
/**
 * 랜딩은 760px에서 마크업이 갈린다 — 좁으면 칩 레일, 넓으면 세로 목록.
 * 두 구현이 같은 tablist 계약을 지키는지 확인해야 하므로 역할로만 찾는다.
 */
...
test('switches the hero and the call to action when another game is picked', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: '출시 알림 받기' })).toBeVisible()
  await expect(page.getByRole('button', { name: '방 만들기' })).toBeHidden()
})
...
test('pulls the room code out of a pasted invite link', async ({ page }) => {
  ...
  const input = page.getByRole('textbox', { name: '방 코드' })
...
test('keeps joining blocked for a link that carries no code', async ({ page }) => {
  ...
  await input.fill('https://yorr.app/join')

  await expect(input).toHaveValue('')
  await expect(page.getByRole('button', { name: '참가' })).toBeDisabled()
})

test('carries the typed code into the invite entry screen', async ({ page }) => {
  await page.goto('/')

  await page.getByRole('textbox', { name: '방 코드' }).fill('yorr64')
